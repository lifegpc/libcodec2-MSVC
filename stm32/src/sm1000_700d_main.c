/*---------------------------------------------------------------------------*\
  ...
  Main program for SM1000.  700D version
...
int main(void) {
    struct freedv *f;
    int            nin, nout, i;
    int            n_samples_ana, n_samples_ana_16k;
    int            n_samples_dv, n_samples_dv_16k;
    ...
    // Define separate sample sizes for modes, experiment
    n_samples_ana = 320;
    n_samples_ana_16k = 2*n_samples_ana;

    /* Set up FreeDV modem */
    f = freedv_open(FREEDV_MODE_700D);
    n_samples_dv = freedv_get_n_speech_samples(f);
    n_samples_dv_16k = 2*n_samples_dv;

    // Make arrays big enough for all
    n_samples = n_samples_ana;
    if (n_samples < n_samples_dv) n_samples = n_samples_dv;
    n_samples_16k = n_samples_ana_16k;
    if (n_samples_16k < n_samples_dv_16k) n_samples_16k = n_samples_dv_16k;

    // To put these arrays into CCM we need fixed sizes.
    // For 700D 
    #define N_SAMPLES 1290
    #define N_SAMPLES_16K (2 * N_SAMPLES)
    static __attribute__ ((section (".ccm")))
    ...
    /* Set up ADCs/DACs */
    #define NUM_ADC_BUFS (floor(N_SAMPLES_16K / ADC_BUF_SZ) + 1)
    #define NUM_DAC_BUFS (floor(N_SAMPLES_16K / DAC_BUF_SZ) + 1)
    dac_open(DAC_FS_16KHZ, NUM_DAC_BUFS * DAC_BUF_SZ);
    adc_open(ADC_FS_16KHZ, NUM_ADC_BUFS * ADC_BUF_SZ);
    ...
    if (!switch_back()) {
        ...
        while(!switch_back()) {
            ...
            if (dac_rem) {
                if (dac_rem > n_samples_ana_16k)
                    dac_rem = n_samples_ana_16k;
...
// TODO: Separate ANA and DV reads????

                if (op_mode == ANALOG) {
                    if (adc2_read(&adc16k[FDMDV_OS_TAPS_16K], n_samples_ana_16k) == 0) {
                        GPIOE->ODR = (1 << 3);

                        /* clipping indicator */
                        led_err(0);
                        for (i=0; i<n_samples_ana_16k; i++) {
                            if (abs(adc16k[FDMDV_OS_TAPS_16K+i]) > 28000)
                                led_err(1);
                        }

                        fdmdv_16_to_8_short(adc8k, &adc16k[FDMDV_OS_TAPS_16K], n_samples_ana);

                        for(i=0; i<n_samples_ana; i++)
                            dac8k[FDMDV_OS_TAPS_8K+i] = adc8k[i];
                        fdmdv_8_to_16_short(dac16k, &dac8k[FDMDV_OS_TAPS_8K], n_samples_ana);
                        dac1_write(dac16k, n_samples_ana_16k);
                    }
                }
                else if (op_mode == DV) {

                    if (adc2_read(&adc16k[FDMDV_OS_TAPS_16K], n_samples_dv_16k) == 0) {
                        GPIOE->ODR = (1 << 3);

                        /* clipping indicator */
                        led_err(0);
                        for (i=0; i<n_samples_dv_16k; i++) {
                            if (abs(adc16k[FDMDV_OS_TAPS_16K+i]) > 28000)
                                led_err(1);
                        }

                        fdmdv_16_to_8_short(adc8k, &adc16k[FDMDV_OS_TAPS_16K], n_samples_dv);

                        freedv_tx(f, &dac8k[FDMDV_OS_TAPS_8K], adc8k);
                        for(i=0; i<n_samples_dv; i++)
                            dac8k[FDMDV_OS_TAPS_8K+i] *= 0.398; /* 8dB back off from peak */
                        fdmdv_8_to_16_short(dac16k, &dac8k[FDMDV_OS_TAPS_8K], n_samples_dv);
                        dac1_write(dac16k, n_samples_dv_16k);
                    }
                }
                else if (op_mode == TONE) {
                        if (!tone_gen.remain)
            ...
            case STATE_RX_TOT:
                ...
                if (op_mode == ANALOG) {
                    ...
                    if (adc1_read(&adc16k[FDMDV_OS_TAPS_16K], n_samples_ana_16k) == 0) {
                        fdmdv_16_to_8_short(adc8k, &adc16k[FDMDV_OS_TAPS_16K], n_samples_ana);
                        for(i=0; i<n_samples_ana; i++)
                            dac8k[FDMDV_OS_TAPS_8K+i] = adc8k[i];
                        fdmdv_8_to_16_short(dac16k, &dac8k[FDMDV_OS_TAPS_8K], n_samples_ana);
                        spk_nsamples = n_samples_ana_16k;
                        led_rt(0); led_err(0);
                   }
                }
                else {
